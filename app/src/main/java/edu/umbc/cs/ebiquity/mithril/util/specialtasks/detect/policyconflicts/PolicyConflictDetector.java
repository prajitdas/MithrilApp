...
public class PolicyConflictDetector {
     ...
     *      d) an action = We are using CWA in this system. We will only state what is allowed. The assumption is that if a particular scenario is not stated explicitly then it's a case when action is to deny
